...
/* harmony default export */ function __WEBPACK_DEFAULT_EXPORT__(app) {
  app.on('bootstrap-before', function () {
    // 加载 later
    var _getHash = (0,circle_core_dom__WEBPACK_IMPORTED_MODULE_1__.getHash)('plugin'),
        hash = _getHash.hash;

    hash.includes('plugin=later') && app.load('later');
  }); // 偏好设置

  ['srt-later'].forEach(function (hook) {
  ...
  app.on('srt-later-add', function () {
    var _ref2 = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {},
        callback = _ref2.callback;

    callback && callback();
    var active = !!app.cache('running');
    var container = active ? app.fire('dom-container') : document.body;
    app.fire('send', 'later-add', {
      active: active,
      url: location.href,
      title: document.title,
      hascode: container.querySelectorAll('pre code').length > 0,
      hasimg: container.querySelectorAll('img[src^="http"]').length > 0
    }, function (_ref3) {
      ...
      app.fire('notice', {
        ...
        btnText: app.i10n('check')
      }, function () {
        if (app.cache('drawer') === 'later') {
          app.fire('guide', 'later', data);
        } else {
          app.fire('srt-later');
        }
      });
