  ...
  function Server(app) {
    ...
    this.INITIAL_VALUES = {
      ...
      "contextmenu": {
        ...
        "ctx-later": {
          "label": app.i10n('later'),
          "checked": true,
          "create": 1627958972277
        },
        "ctx-later-add": {
          "label": app.i10n('later_add'),
          "checked": true,
          "create": 1627958972278
        }
      }
    };
  }
...
/* harmony export */ });
/* harmony import */ var circle_core_is__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! circle/core/is */ "../circle/core/is.js");

/* harmony default export */ function __WEBPACK_DEFAULT_EXPORT__(app) {
  // 看门狗
  app.on('watchdog', function (_ref) {
    var callback = _ref.callback;
    callback && callback(); // 两天检查一下消息和用户角色
  ...
  ['open-later'].forEach(function (hook) {
  ...
  app.on('later-add', function () {
    var _ref3 = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {},
        request = _ref3.request,
        _callback = _ref3.callback;
    ...
    app.fire('db', {
      ...
      callback: function callback(error, data) {
        ...
        if (data) {
          _callback && _callback("\"".concat(data.title, "\" ").concat(app.i10n('existed')), data.key);
          return;
        }

        app.fire('db', {
          callback: function callback(error, data) {
            _callback && _callback(error, data);
            app.fire('send', 'later-refetch');
          },
          request: {
            execute: 'add',
            table: 'later',
  ...
  app.on('ctx-click', function (info, tab) {
    ...
    switch (info.menuItemId) {
      case 'ctx-later-add':
        app.fire('send', 'srt-later-add', {
          tab: tab
        });
        break;

      case 'ctx-later':
        if ((0,circle_core_is__WEBPACK_IMPORTED_MODULE_0__.isString)(url) && url.startsWith('chrome')) {
          app.fire('open-later');
        } else {
          app.fire('send', 'srt-later', {
            tab: tab
          });
        }
...
Bridge.register('app', ['database', 'wrench', 'message'], function () {
  ...
  (0,includes_app_server__WEBPACK_IMPORTED_MODULE_1__["default"])(app); // 初始化右键菜单

  server.contextmenu(app);
});
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = self["webpackChunkaddon"] = self["webpackChunkaddon"] || [];
